...
(привязка по token — alias записи в `installed_apps`).

> ⚠️ **Переустановка приложения уничтожает данные storage.**
> При удалении и повторной установке создаётся новая запись в `installed_apps` с новым `token`.
> Старые записи в `apps_storage` остаются осиротевшими и недоступными для нового `token`.
> Если данные важны — предусмотри экспорт/миграцию перед удалением, либо используй
> **кастомный каталог** (`custom_dbtables`): он привязан к группе и переживает переустановку.
