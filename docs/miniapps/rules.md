...
- **Никогда** не захардкоживай URL инстанса (`panel.korfix.info` и т.п.) — пиши относительные пути, чтобы приложение работало на любом инстансе.
...
- **Не ссылайся на конкретный домен** ни в коде миниапа, ни в `config.json`, если это локальное приложение. Миниап должен работать на `panel.korfix.info`, `acme.korfix.info` и любом другом инстансе без правок.
