...
- [ ] **`install`-фрейм** (если есть self-provisioning):
  - [ ] Каждый мутирующий `App.fetch` → статус ответа проверяется: `if (!resp || resp.status === 'error' || resp.status === 'no') throw new Error(...)`
...
- [ ] **Права `access_db` — схема доступа осознанно выбрана**:
    - Платформа **автоматически создаёт** запись access_db (root+adm=1, остальные=0) сразу после INSERT в custom_dbtables — не нужно создавать, нужно обновлять
